...
function QuestionsInner() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const [{ data, error }, { data: lesson }] = await Promise.all([
        supabase
          .from("questions")
          .select("*")
  ...
  const handleApprove = async (q: Question) => {
    ...
    if (!error)
      setQuestions((prev) =>
        prev.map((x) => (x.id === q.id ? { ...x, is_approved: true } : x)),
      );
    setSavingId(null);
  };
  ...
  const handleUnapprove = async (q: Question) => {
    ...
    if (!error)
      setQuestions((prev) =>
        prev.map((x) => (x.id === q.id ? { ...x, is_approved: false } : x)),
      );
    setSavingId(null);
  };
      ...
      <div style={s.page}>
        ...
        <div style={s.pageHeader}>
          <div>
            ...
            <p style={s.pageDesc}>
              Edit or regenerate hints, then approve questions before publishing
              the lesson. Editing a question resets its approval status.
            </p>
          </div>
            ...
            {totalCount > 0 && (
              <button
                ...
                style={{
                  ...
                  border: isPublished
                    ? "1.5px solid rgba(139,26,26,0.2)"
                    : "none",
                  borderRadius: "9px",
        ...
        {!loading &&
          ...
          DIFFICULTY_ORDER.map((diff) => {
            ...
            const groupApproved = grouped[diff].filter(
              (q) => q.is_approved,
            ).length;
            return (
                ...
                <div style={s.groupHeader}>
                  ...
                  <div style={s.groupMeta}>
                    <span style={s.groupCount}>
                      {groupApproved} / {grouped[diff].length} approved
                    </span>
                      ...
                      {isEditing && editState ? (
                        <div>
                          <div style={s.editNote}>
                            Editing will reset approval status for this
                            question.
                          </div>
                          ...
                          <div style={s.field}>
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setEditState((st) =>
                                  st
                                    ? { ...st, question_text: e.target.value }
                                    : st,
                                )
                              }
                              style={s.textarea}
                          ...
                          <div style={{ ...s.field, maxWidth: "200px" }}>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setEditState((st) =>
                                  st
                                    ? { ...st, correct_answer: e.target.value }
                                    : st,
                                )
                              }
                              style={s.select}
                          ...
                          <div style={s.field}>
                            <label style={s.label}>
                              ...
                              >
                                — shown when student answers wrong
                              </span>
                            </label>
                            <textarea
                              className="qr-textarea"
...
const s: Record<string, React.CSSProperties> = {
  ...
  spinner: {
    ...
    height: "22px",
    borderRadius: "50%",
    border: "3px solid rgba(26,122,64,0.15)",
  ...
  emptyGroup: {
    ...
    background: "#fff",
    border: "1.5px solid rgba(26,122,64,0.1)",
    borderRadius: "12px",
  },
  ...
  hintBox: {
    ...
    alignItems: "flex-start",
    background: "#fffbf0",
    border: "1.5px solid rgba(200,130,0,0.2)",
    borderRadius: "9px",
