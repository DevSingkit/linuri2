...
export default function NewLessonPage() {
  ...
  const uploadFile = async (file: File): Promise<string> => {
    ...
    const path = `lesson-files/${Date.now()}.${ext}`;
  ...
  const handleSubmit = async () => {
    ...
    if (!skillName.trim()) {
      setError("Target skill is required.");
      return;
    }
    if (!lessonText.trim()) {
    ...
    if (lessonText.trim().length < 100) {
      setError(
        "Lesson content is too short. Add more detail (min 100 characters).",
      );
      return;
    }
    if (!classId) {
    ...
    try {
      ...
      if (lessonError || !lesson)
        throw new Error(lessonError?.message ?? "Failed to save lesson.");
      ...
      <AppLayout title="New Lesson">
        ...
        <div style={s.center}>
          <div style={s.genCard}>
            ...
            <p style={s.genDesc}>
              Gemini AI is creating 15 questions across Basic, Standard, and
              Advanced difficulty. This takes a few seconds.
            </p>
            <div style={s.genDots}>
              {[0, 1, 2].map((i) => (
                <div
      ...
      <AppLayout title="New Lesson">
        <div style={s.center}>
          <div style={{ ...s.genCard, borderColor: "rgba(26,122,64,0.3)" }}>
            ...
            <p style={s.genDesc}>
              Redirecting you to review and approve the questions…
            </p>
          </div>
        </div>
      </AppLayout>
      ...
      <div className="nl-page" style={s.page}>
        ...
        <div style={s.topRow}>
          <div>
            ...
            <p style={s.muted}>
              Fill in the lesson details. Gemini AI will generate 15 questions
              automatically.
            </p>
          </div>
          <button
        ...
        <div style={s.formCard}>
          ...
          <div style={s.field}>
            <label style={s.label}>
              Lesson Title <span style={s.req}>*</span>
            </label>
            <input
              className="nl-input"
              style={s.input}
          ...
          <div style={s.field}>
            <label style={s.label}>
              Target Skill <span style={s.req}>*</span>
            </label>
            <input
              className="nl-input"
              style={s.input}
          ...
          <div style={s.field}>
            <label style={s.label}>
              Starting Difficulty <span style={s.req}>*</span>
            </label>
            <div style={s.diffRow}>
              {DIFFICULTIES.map((d) => (
                <button
            ...
            <p style={s.hint}>
              This sets the default difficulty stored with the lesson. Gemini
              generates all 3 difficulty levels regardless.
            </p>
          </div>
          ...
          <div style={s.field}>
            <label style={s.label}>
              ...
              <span
                style={{ ...s.hint, display: "inline", marginLeft: "0.5rem" }}
              >
                — {lessonText.length} characters{" "}
                {lessonText.length < 100 ? "(min 100)" : "✓"}
              </span>
            </label>
            <textarea
              className="nl-input"
          ...
          <div style={s.field}>
            <label style={s.label}>
              ...
              >
                (optional — PDF, Word, PowerPoint, Excel, etc.)
              </span>
            </label>
          ...
          <div style={s.infoStrip}>
            ...
            <div>
              ...
              >
                ...
                <strong>15 multiple-choice questions</strong> (5 Basic · 5
                Standard · 5 Advanced) based on your lesson content. You will
                then review and approve each question before students can see
                them.
              </div>
            </div>
          </div>
          ...
          >
            <button
              ...
              disabled={step !== "form"}
            >
              {uploading ? "Uploading file…" : "Generate Questions →"}
            </button>
            <button
              className="nl-cancel"
              style={s.btnCancel}
...
const s: Record<string, React.CSSProperties> = {
  ...
  genCard: {
    ...
    width: "100%",
    textAlign: "center",
    boxShadow: "0 4px 24px rgba(13,61,32,0.07)",
    display: "flex",
    flexDirection: "column",
    alignItems: "center",
    gap: "1rem",
  ...
  spinner: {
    ...
    borderRadius: "50%",
    animation: "nl-spin 0.85s linear infinite",
  },
  fileLabel: {
    display: "flex",
    alignItems: "center",
    gap: "0.75rem",
    background: "#fdfaf5",
    border: "1.5px dashed rgba(26,122,64,0.25)",
    borderRadius: "10px",
