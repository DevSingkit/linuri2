...
export default function TeacherReportsPage() {
  ...
  const subjectMeta: Record<string, { icon: string }> = {
    English: { icon: '📖' }, Mathematics: { icon: '🔢' }, Science: { icon: '🔬' },
  }
    ...
    <AppLayout title="Reports">
      ...
      <div style={s.page}>
        ...
        <div style={s.topRow}>
          ...
          <button className="rp-print" style={s.btnPrint} onClick={() => window.print()}>
            🖨️ Print / Save PDF
          </button>
        </div>
        ...
        {total > 0 && (
          <div style={s.summaryCard}>
            <div style={s.barLabel}>
              <span style={{ fontWeight: 600, color: '#0d3d20', fontSize: '0.95rem' }}>Overall Mastery</span>
            ...
            <div style={s.bar}>
              ...
              {developing > 0 && <div style={{ ...s.barSeg, width: `${Math.round((developing / total) * 100)}%`, background: 'linear-gradient(90deg,#d4a017,#b07800)' }}>{Math.round((developing / total) * 100)}%</div>}
              {needsHelp  > 0 && <div style={{ ...s.barSeg, width: `${Math.round((needsHelp  / total) * 100)}%`, background: 'linear-gradient(90deg,#c0392b,#8b1a1a)' }}>{Math.round((needsHelp  / total) * 100)}%</div>}
            </div>
          ...
          {(['English', 'Mathematics', 'Science'] as Subject[]).map(sub => {
            ...
            return (
              <div key={sub} className="rp-subcard" style={s.subjectCard}>
                ...
                <div style={{ height: '5px', borderRadius: '99px', background: '#e5e7eb', overflow: 'hidden', margin: '0.4rem 0 0.6rem' }}>
                  <div style={{ height: '100%', width: `${rows.length ? Math.round((rows.filter(m => m.mastery_level === 'Mastered').length / rows.length) * 100) : 0}%`, background: '#1a7a40', borderRadius: '99px' }} />
                </div>
                <div style={s.subjectRow}><span style={{ color: '#0d5c28', fontWeight: 500 }}>⭐ Mastered</span><strong style={{ color: '#0d5c28' }}>{rows.filter(m => m.mastery_level === 'Mastered').length}</strong></div>
                <div style={s.subjectRow}><span style={{ color: '#7a5500', fontWeight: 500 }}>📈 Developing</span><strong style={{ color: '#7a5500' }}>{rows.filter(m => m.mastery_level === 'Developing').length}</strong></div>
                <div style={s.subjectRow}><span style={{ color: '#8b1a1a', fontWeight: 500 }}>🆘 Needs Help</span><strong style={{ color: '#8b1a1a' }}>{rows.filter(m => m.mastery_level === 'Needs Help').length}</strong></div>
              </div>
            )
          })}
        </div>
        ...
        <div style={s.filterRow}>
          <div style={s.filterGroup}>
            ...
            <select className="rp-select" style={s.select} value={selectedClass} onChange={e => setSelectedClass(e.target.value)}>
        ...
        <p style={{ ...s.muted, marginBottom: '0.75rem' }}>
          Showing <strong>{filtered.length}</strong> of {total} record{total !== 1 ? 's' : ''}
        </p>
        ...
        {mastery.length === 0 ? (
          <div style={s.empty}>
            ...
            <p style={{ margin: 0, color: '#6b7280', fontSize: '0.9rem' }}>Students need to complete quizzes first.</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={s.empty}>
            <span style={{ fontSize: '2.5rem' }}>🔍</span>
            <p style={{ margin: 0, fontWeight: 600, color: '#0d3d20' }}>No records match your filters.</p>
          </div>
        ) : (
            ...
            <table style={s.table}>
              ...
              <tbody>
                {filtered.map((m, i) => {
                  ...
                  return (
                    <tr key={i} className="rp-tr" style={i % 2 === 0 ? s.trEven : s.trOdd}>
                      ...
                      <td style={{ ...s.td, textAlign: 'center' as const }}>
                        <span style={{ fontWeight: m.regression_count >= 2 ? 700 : 400, color: m.regression_count >= 2 ? '#8b1a1a' : '#6b7280', background: m.regression_count >= 2 ? '#fff0f0' : 'transparent', padding: m.regression_count >= 2 ? '2px 10px' : '0', borderRadius: '5px', fontSize: '0.9rem' }}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  btnPrint:    { display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#0d3d20', color: '#ffd166', border: 'none', borderRadius: '9px', padding: '0.75rem 1.5rem', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(13,61,32,0.18)', transition: 'background 0.15s, transform 0.15s' },
  summaryCard: { background: '#fff', border: '1.5px solid rgba(26,122,64,0.13)', borderRadius: '18px', padding: '1.5rem', marginBottom: '1.5rem', boxShadow: '0 2px 10px rgba(13,61,32,0.04)' },
  barLabel:    { display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' },
  bar:         { height: '32px', borderRadius: '10px', background: '#e5e7eb', display: 'flex', overflow: 'hidden', marginBottom: '0.75rem' },
  barSeg:      { height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700, color: '#fff', minWidth: '36px' },
  barLegend:   { display: 'flex', gap: '1.25rem', fontSize: '0.88rem', flexWrap: 'wrap' as const, fontWeight: 500, color: '#1a1f16' },
  ...
  subjectGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: '1rem', marginBottom: '1.75rem' },
  subjectCard: { background: '#fff', border: '1.5px solid rgba(26,122,64,0.13)', borderRadius: '18px', padding: '1.35rem', display: 'flex', flexDirection: 'column', gap: '0.4rem', cursor: 'default' },
  ...
  subjectRow:  { display: 'flex', justifyContent: 'space-between', fontSize: '0.88rem', alignItems: 'center' },
  filterRow:   { display: 'flex', gap: '1rem', flexWrap: 'wrap' as const, alignItems: 'flex-end', marginBottom: '1rem' },
  filterGroup: { display: 'flex', flexDirection: 'column' as const, gap: '0.4rem' },
  filterLabel: { fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#6b7280' },
  filterBtns:  { display: 'flex', gap: '0.35rem', flexWrap: 'wrap' as const },
  fBtn:        { background: '#fff', border: '1.5px solid rgba(26,122,64,0.15)', borderRadius: '8px', padding: '0.5rem 1rem', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer', color: '#6b7280', fontFamily: 'inherit', transition: 'background 0.15s, color 0.15s' },
  fBtnActive:  { background: '#0d3d20', color: '#ffd166', borderColor: '#0d3d20' },
  select:      { border: '1.5px solid rgba(26,122,64,0.2)', borderRadius: '9px', padding: '0.6rem 1rem', fontSize: '0.95rem', fontFamily: 'inherit', outline: 'none', color: '#1a1f16', background: '#fdfaf5', cursor: 'pointer', transition: 'border-color 0.15s' },
  search:      { border: '1.5px solid rgba(26,122,64,0.2)', borderRadius: '9px', padding: '0.6rem 1rem', fontSize: '0.95rem', fontFamily: 'inherit', outline: 'none', minWidth: '220px', color: '#1a1f16', background: '#fdfaf5', transition: 'border-color 0.15s' },
  ...
  spinner:     { width: '42px', height: '42px', border: '4px solid #eaf6ef', borderTop: '4px solid #1a7a40', borderRadius: '50%', animation: 'rp-spin 0.8s linear infinite' },
}