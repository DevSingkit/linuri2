  ...
  return (
    <AppLayout title="Alerts">
      ...
      <div style={s.page}>
        ...
        <div style={s.chips}>
          {[
            { label: 'Critical (≥3)', value: criticalCount, icon: '🔴', bg: '#fff0f0', color: '#8b1a1a', border: 'rgba(155,28,28,0.18)' },
            { label: 'Warning (×2)',  value: warningCount,  icon: '🟡', bg: '#fffbf0', color: '#7a5500', border: 'rgba(200,130,0,0.20)' },
            { label: 'Total Flagged', value: alerts.length, icon: '⚠️', bg: '#fdfaf5', color: '#0d3d20', border: 'rgba(26,122,64,0.13)' },
          ].map(c => (
            <div key={c.label} className="al-chip" style={{ ...s.chip, background: c.bg, border: `1.5px solid ${c.border}` }}>
        ...
        {alerts.length === 0 ? (
          ...
          <>
              ...
              {(['All', 'Critical', 'Warning'] as SeverityFilter[]).map(f => (
                <button
                  ...
                  style={{
                    ...s.fBtn,
                    ...(severityFilter === f
                      ? f === 'Critical' ? s.fBtnCritical
                        : f === 'Warning' ? s.fBtnWarning
                        : s.fBtnActive
                      : {}),
                  }}
            ...
            <p style={{ ...s.muted, marginBottom: '0.75rem' }}>
              Showing <strong>{filtered.length}</strong> of {alerts.length} flagged record{alerts.length !== 1 ? 's' : ''}
            </p>
                ...
                <table style={s.table}>
                  ...
                  <tbody>
                    {filtered.map((a, i) => {
                      ...
                      const mm = masteryMeta[a.mastery_level] ?? masteryMeta['Needs Help']
                      return (
                        <tr key={i} className="al-tr" style={i % 2 === 0 ? s.trEven : s.trOdd}>
                          <td style={s.td}>
                          ...
                          <td style={{ ...s.td, fontWeight: 600 }}>
                            <div style={s.avatarRow}>
                              <div style={{ ...s.avatarDot, background: isCritical ? '#fff0f0' : '#fffbf0', color: isCritical ? '#8b1a1a' : '#7a5500' }}>
