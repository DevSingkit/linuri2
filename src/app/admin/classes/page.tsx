...
interface ClassRow {
  ...
  join_code: string; teacher_id: string; created_at: string
...
export default function AdminClassesPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const { data: ed, error: ee } = await supabase.from('enrollments').select('class_id')
        if (ee) throw ee
        const m: Record<string, number> = {}
        ;(ed ?? []).forEach((e: { class_id: string }) => { m[e.class_id] = (m[e.class_id] ?? 0) + 1 })
        setCounts(m)
  ...
  if (loading) return <AppLayout title="Classes"><div style={S.center}><div style={S.spin} /><p style={S.muted}>Loading…</p></div></AppLayout>
  if (error)   return <AppLayout title="Classes"><div style={S.center}><div style={S.errCard}><p style={{ color: '#8b1a1a', fontWeight: 600 }}>{error}</p></div></div></AppLayout>
    ...
    <AppLayout title="Classes">
      ...
      <div className="pg">
        ...
        <div style={{ position: 'relative', marginBottom: '1.5rem', maxWidth: '480px' }}>
          ...
          <input className="search-box" type="text" placeholder="Search section, teacher, or code…" value={search} onChange={e => setSearch(e.target.value)} />
        </div>

        <p style={{ ...S.muted, marginBottom: '1rem' }}>
          Showing <strong style={{ color: '#0d3d20' }}>{filtered.length}</strong> of {classes.length} classes
        </p>

        {filtered.length === 0 ? (
          <div style={S.empty}><span style={{ fontSize: '2.5rem' }}>🏫</span><p style={{ fontWeight: 600, color: '#0d3d20' }}>No classes found</p><p style={S.muted}>Try a different search.</p></div>
        ) : (
          <div style={S.grid}>
            {filtered.map(c => {
              ...
              return (
                <div key={c.id} className="cls-card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                    <div style={{ fontFamily: "'Lora', serif", fontSize: '1.1rem', fontWeight: 700, color: '#0d3d20', lineHeight: 1.25 }}>{c.name} — {c.section}</div>
                    <span style={S.badge}>{n} student{n !== 1 ? 's' : ''}</span>
                  </div>
                  ...
                  <div style={S.cardField}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.25rem' }}>
                      ...
                      <button className={`copy-btn${copied === c.join_code ? ' done' : ''}`} onClick={() => copyCode(c.join_code)}>
                        {copied === c.join_code ? '✓ Copied' : 'Copy'}
                      </button>
                    </div>
                  </div>

                  <div style={{ fontSize: '0.76rem', color: '#9ca3af', borderTop: '1px solid rgba(26,122,64,0.08)', paddingTop: '0.65rem' }}>
...
const S: Record<string, React.CSSProperties> = {
  ...
  badge:    { background: '#eaf6ef', color: '#0d3d20', fontSize: '0.72rem', fontWeight: 700, padding: '0.22rem 0.65rem', borderRadius: '20px', border: '1px solid rgba(26,122,64,0.18)', whiteSpace: 'nowrap', flexShrink: 0 },
  cardField:{ display: 'flex', flexDirection: 'column', gap: '0.15rem' },
  cardLabel:{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#6b7280' },
  ...
  empty:    { background: '#fff', border: '1.5px solid rgba(26,122,64,0.13)', borderRadius: '14px', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem', color: '#6b7280', textAlign: 'center' },
