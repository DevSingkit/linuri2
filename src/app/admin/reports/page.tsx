...
interface MasteryRow {
  student_id: string; skill_name: string; subject: Subject
  mastery_level: MasteryLevel; difficulty_level: string
  regression_count: number; updated_at: string
...
export default function AdminReportsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const { data, error: e } = await supabase.from('mastery_history').select('*, users(name)').order('updated_at', { ascending: false })
        if (e) throw e
  ...
  const filtered = mastery.filter(m =>
    ...
    (search === '' || (m.users?.name ?? '').toLowerCase().includes(search.toLowerCase()) || m.skill_name.toLowerCase().includes(search.toLowerCase()))
  )
  ...
  if (loading) return <AppLayout title="Reports"><div style={S.center}><div style={S.spin} /><p style={S.muted}>Loading…</p></div></AppLayout>
  if (error)   return <AppLayout title="Reports"><div style={S.center}><div style={S.errCard}><p style={{ color: '#8b1a1a', fontWeight: 600 }}>{error}</p></div></div></AppLayout>
    ...
    <AppLayout title="Reports">
      ...
      <div className="pg">
        ...
        <div style={S.topRow}>
          ...
          <button style={S.printBtn} onClick={() => window.print()}>🖨️ Print / PDF</button>
        </div>
        ...
        {total > 0 && (
          <div style={S.summaryCard}>
            <div style={S.bar}>
              {mastered > 0 && <div style={{ ...S.barSeg, width: `${Math.round(mastered/total*100)}%`, background: 'linear-gradient(90deg,#1a7a40,#0d5c28)' }}>{Math.round(mastered/total*100)}%</div>}
              {develop  > 0 && <div style={{ ...S.barSeg, width: `${Math.round(develop/total*100)}%`,  background: 'linear-gradient(90deg,#d4a017,#b07800)' }}>{Math.round(develop/total*100)}%</div>}
              {needs    > 0 && <div style={{ ...S.barSeg, width: `${Math.round(needs/total*100)}%`,    background: 'linear-gradient(90deg,#c0392b,#8b1a1a)' }}>{Math.round(needs/total*100)}%</div>}
            </div>
            <div style={S.barLegend}>
              <span style={{ color: '#1a7a40', fontWeight: 600, fontSize: '0.82rem' }}>● Mastered ({mastered})</span>
              <span style={{ color: '#d4a017', fontWeight: 600, fontSize: '0.82rem' }}>● Developing ({develop})</span>
              <span style={{ color: '#c0392b', fontWeight: 600, fontSize: '0.82rem' }}>● Needs Help ({needs})</span>
              {flagged > 0 && <span style={{ color: '#8b1a1a', fontWeight: 700, fontSize: '0.82rem' }}>⚠ {flagged} flagged</span>}
            </div>
          </div>
        )}
          ...
          {(['English', 'Mathematics', 'Science'] as Subject[]).map(sub => {
            ...
            return (
              <div key={sub} style={S.subCard}>
                ...
                {[['⭐ Mastered', m2, '#0d5c28'], ['📈 Developing', d2, '#7a5500'], ['🆘 Needs Help', n2, '#8b1a1a']].map(([label, val, clr]) => (
                  <div key={label as string} style={{ display:'flex', justifyContent:'space-between', fontSize:'0.82rem', marginBottom:'0.2rem' }}>
                    <span style={{ color: clr as string, fontWeight: 500 }}>{label as string}</span>
        ...
        <div style={S.filterWrap}>
          <div style={{ position:'relative', flex:1, minWidth:0 }}>
            ...
            <input className="search-box" type="text" placeholder="Search student or skill…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div style={S.filterGroups}>
            <div style={S.filterGroup}>
              <span style={S.filterLabel}>Subject</span>
              <div style={{ display:'flex', gap:'0.35rem', flexWrap:'wrap' }}>
                {(['All','English','Mathematics','Science'] as SF[]).map(f => (
                  <button key={f} className={`fbtn${sf===f?' on':''}`} onClick={() => setSf(f)}>{f}</button>
                ))}
              </div>
            </div>
            <div style={S.filterGroup}>
              ...
              <div style={{ display:'flex', gap:'0.35rem', flexWrap:'wrap' }}>
                {(['All','Mastered','Developing','Needs Help'] as MF[]).map(f => (
                  <button key={f} className={`fbtn${mf===f?' on':''}`} onClick={() => setMf(f)}>{f}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <p style={{ ...S.muted, marginBottom:'1rem' }}>
          Showing <strong style={{ color:'#0d3d20' }}>{filtered.length}</strong> of {total} records
        </p>

        {filtered.length === 0 ? (
          <div style={S.empty}><span style={{ fontSize:'2rem' }}>📭</span><p style={{ fontWeight:600, color:'#0d3d20' }}>No records match</p></div>
        ) : (
          <>
            {/* Desktop table */}
              ...
              <table style={S.table}>
                ...
                <tbody>
                  {filtered.map((m, i) => {
                    ...
                    return (
                      <tr key={i} className="rpt-tr" style={i%2===0?S.trEven:S.trOdd}>
                        ...
                        <td style={S.td}>
                          <span style={{ ...S.pill, background:ms.bg, color:ms.color, border:`1px solid ${ms.border}` }}>{m.mastery_level}</span>
                        </td>
                ...
                return (
                  <div key={i} style={S.rCard}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'0.5rem' }}>
                      <div>
                        <div style={{ fontWeight:700, color:'#0d3d20', fontSize:'0.95rem' }}>{m.users?.name ?? '—'}</div>
                        <div style={{ fontSize:'0.82rem', color:'#6b7280', marginTop:'2px' }}>{m.skill_name}</div>
                      </div>
                      <span style={{ ...S.pill, background:ms.bg, color:ms.color, border:`1px solid ${ms.border}`, flexShrink:0 }}>{m.mastery_level}</span>
                    </div>
                    <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', fontSize:'0.8rem', color:'#6b7280', marginTop:'0.25rem' }}>
                      <span>{m.subject}</span><span>·</span><span>{m.difficulty_level}</span>
                      {m.regression_count >= 2 && <span style={{ color:'#8b1a1a', fontWeight:700 }}>⚠ {m.regression_count} regressions</span>}
                      <span style={{ marginLeft:'auto' }}>{new Date(m.updated_at).toLocaleDateString('en-PH',{month:'short',day:'numeric',year:'numeric'})}</span>
                    </div>
                  </div>
                )
              })}
...
const S: Record<string, React.CSSProperties> = {
  ...
  summaryCard: { background:'#fff', border:'1.5px solid rgba(26,122,64,0.13)', borderRadius:'14px', padding:'1.35rem 1.5rem', marginBottom:'1.25rem' },
  bar:         { height:'28px', borderRadius:'8px', background:'#e5e7eb', display:'flex', overflow:'hidden', marginBottom:'0.75rem' },
  ...
  subGrid:     { display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:'1rem', marginBottom:'1.5rem' },
  subCard:     { background:'#fff', border:'1.5px solid rgba(26,122,64,0.13)', borderRadius:'14px', padding:'1.1rem 1.25rem' },
  filterWrap:  { display:'flex', flexDirection:'column', gap:'0.75rem', marginBottom:'1.1rem' },
  filterGroups:{ display:'flex', gap:'1rem', flexWrap:'wrap' },
  filterGroup: { display:'flex', flexDirection:'column', gap:'0.35rem' },
  filterLabel: { fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'#6b7280' },
  ...
  table:       { width:'100%', borderCollapse:'collapse', fontSize:'0.9rem', minWidth:'640px' },
  ...
  pill:        { fontSize:'0.73rem', fontWeight:700, padding:'0.25rem 0.65rem', borderRadius:'20px', whiteSpace:'nowrap' },
  rCard:       { background:'#fff', border:'1.5px solid rgba(26,122,64,0.13)', borderRadius:'14px', padding:'1rem 1.1rem', display:'flex', flexDirection:'column', gap:'0.4rem' },
  empty:       { background:'#fff', border:'1.5px solid rgba(26,122,64,0.13)', borderRadius:'14px', padding:'2.5rem', color:'#6b7280', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.75rem' },
  center:      { display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'60vh', gap:'1rem' },
  errCard:     { background:'#fff0f0', border:'1.5px solid rgba(155,28,28,0.18)', borderRadius:'14px', padding:'2rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.75rem' },
