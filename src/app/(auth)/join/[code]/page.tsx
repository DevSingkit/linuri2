...
export default function JoinPage() {
  ...
  const [classInfo, setClassInfo] = useState<{ id: string; name: string; section: string } | null>(null)
  ...
  const handleJoin = async () => {
    ...
    if (!/^\d{12}$/.test(lrn.trim())) { setError('LRN must be exactly 12 digits.'); return }
    if (password.length < 6) { setError('Password must be at least 6 characters.'); return }
    if (password !== confirmPassword) { setError('Passwords do not match.'); return }
    if (!classInfo) { setError('Invalid join code.'); return }
    ...
    if (updateError) {
      setError('Account created but profile update failed. Contact your teacher.')
      setLoading(false)
      return
    }
  ...
  return (
    <>
      ...
      <div className="join-page">
        {checkingCode ? (
          <div style={{ zIndex: 1, position: 'relative', color: '#6b7280', fontFamily: 'var(--font)' }}>
            Checking join code…
          </div>
        ) : classError ? (
          <div className="join-invalid">
            <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>🔗</div>
            <h2 style={{ color: '#8b1a1a', fontWeight: 800, marginBottom: '0.5rem' }}>Invalid Join Link</h2>
            <p style={{ color: '#6b7280', fontSize: '0.9rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              This join link is invalid or has expired. Ask your teacher for a new one.
            </p>
            <a href="/login" style={{ color: '#1a7a40', fontWeight: 700, textDecoration: 'none' }}>
              ← Back to login
            </a>
          </div>
        ) : (
          <div className="join-card">
            <div className="join-logo">
              <div className="join-mark">L</div>
              <span className="join-mark-name">LINURI</span>
              <span className="join-mark-badge">Grade 6</span>
            </div>
            ...
            <p className="join-sub">Fill in your details to join your class. Use your LRN to log in next time.</p>
            ...
            <div className="join-field">
              <label className="join-label">
                LRN <span className="join-label-hint">— 12-digit Learner Reference Number</span>
              </label>
              <input
                type="text"
                value={lrn}
            ...
            <div className="join-field">
              <label className="join-label">
                Password <span className="join-label-hint">— at least 6 characters</span>
              </label>
              <input
                type="password"
                value={password}
